            ...
            <section className="max-w-7xl mx-auto px-4 py-24">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {universities.map((uni, i) => (
                        <div key={i} className="border border-black dark:border-gray-700 p-8 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all group cursor-default">
                            <div className="w-16 h-16 mb-6 bg-gray-100 dark:bg-gray-800 text-black dark:text-white flex items-center justify-center rounded-none group-hover:bg-white dark:group-hover:bg-black transition-colors">
                                <GraduationCap className="w-8 h-8" />
                            </div>
                            <h3 className="text-2xl font-bold uppercase mb-2">{uni.name}</h3>
                            <div className="flex items-center text-sm font-bold uppercase tracking-widest text-gray-500 group-hover:text-gray-300 dark:group-hover:text-gray-600 mb-4">
                                <Globe className="w-4 h-4 mr-2" /> {uni.location}
                            </div>
                            <p className="text-gray-600 dark:text-gray-400 group-hover:text-gray-400 dark:group-hover:text-gray-500 leading-relaxed font-light">
                                {uni.description}
                            </p>
                        </div>
                    ))}
