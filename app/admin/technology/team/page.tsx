...
const categoryColors: Record<string, string> = {
    ...
    it_team: "bg-blue-100 text-blue-800",
    support_team: "bg-green-100 text-green-800",
};
...
export default function AdminTechTeamPage() {
    ...
    const grouped = sections.reduce((acc, cat) => {
        acc[cat] = members.filter(m => getCategory(m) === cat);
        return acc;
        ...
        <div className="p-8">
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-gray-500">Manage your tech team by section.</p>
                </div>
                <Link href="/admin/technology/team/new"
            ...
            {loading ? (
                ...
                <div className="text-center text-gray-400 italic py-12">No team members found. Add one to get started.</div>
            ) : (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    {catMembers.map((member: any, memberIdx: number) => (
                                        <div key={member.id} className="bg-white border border-gray-200 p-6 flex flex-col items-center text-center shadow-sm relative group">
                                            ...
                                            <span className={`absolute top-3 left-3 text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded ${categoryColors[getCategory(member)]}`}>
                                                {categoryLabels[getCategory(member)]}
                                            </span>
